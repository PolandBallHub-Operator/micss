...
**Micss** は、フレームワークやビルドツールを必要としない、軽量な Web Components UI ライブラリです。HTML に JavaScript を1つ読み込むだけで、カード、ボタン、入力欄、トグル、スライダー、ナビゲーション、トースト、ダイアログなどを利用できます。

この README は、Shadow DOM の構築不具合を修正した **fixed 版**を対象にしています。
...
運用環境では、`main` のような可変ブランチではなく、リリースタグまたはコミット SHA に固定することを推奨します。

```html
<script src="https://cdn.jsdelivr.net/gh/PolandBallHub-Operator/micss@<commit-SHA>/micss.js"></script>
```

開発中に最新版を確認する場合は、次の URL も利用できます。

```html
<script src="https://cdn.jsdelivr.net/gh/PolandBallHub-Operator/micss@main/micss.js"></script>
```
...
ライセンスを明示する場合は、リポジトリの `LICENSE` ファイルを正としてください。README だけではライセンス条件を確定できません。
...
- [jsDelivr: Micss distribution](https://cdn.jsdelivr.net/gh/PolandBallHub-Operator/micss@main/micss.js)
